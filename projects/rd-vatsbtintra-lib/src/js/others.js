...
function paste(el) {
  if (window.clipboardData) {
    ...
    else if (el.innerHTML.length < 1) el.innerHTML = "&nbsp;"; // iOS needs element not to be empty to select it and pop up 'paste' button
    ...
    if (document.queryCommandSupported("paste")) {
      ...
      if (successful) tooltip(el, "Pasted.");
        ...
        } else tooltip(el, "Press CTRL-V to paste");
      }
    } else {
...
$(document).ready(function () {
  ...
  $("#collapseExample .sub2").on("shown.bs.collapse", function () {
    $(".nav-link[aria-controls=" + $(this).attr("id") + "]").addClass("active");
  });

  $("#collapseExample .sub2").on("hidden.bs.collapse", function () {
    $(".nav-link[aria-controls=" + $(this).attr("id") + "]").removeClass(
      "active"
    );
  });

  $("#collapseExample .nav-link[data-toggle=collapse]").on(
    "click",
    function () {
      $("#collapseExample")
        .find(".nav-link[data-toggle=collapse]")
        .removeClass("active");
      $("#collapseExample .sub2").collapse("hide");
      $(this).addClass("active");
    }
  );

  $("#collapseExample").on("shown.bs.collapse", function () {
    $("#collapseExample8").collapse("hide");
  });
  ...
  $("#collapseExample8").on("shown.bs.collapse", function () {
    $("#collapseExample").collapse("hide");
  });

  $("#collapseExample8").on("hidden.bs.collapse", function () {
    if ($(this).hasClass("show") == false) {
      $("#collapseExample8 .sub2").collapse("hide");
    }
  });

  $(".sub2 .flex-column .nav-link").on("click", function (e) {
    $(".sub2 .flex-column").find(".active").removeClass("active");
  });

  $(".first-nav").on("click", function (e) {
    if ($(this).attr("data-toggle") !== "collapse") {
      $(".flex-column").find(".active").removeClass("active");
    //   $(".sub1").collapse("hide");
    //   $(".sub2").collapse("hide");
    } else {
      $(".flex-column").find(".first-nav").removeClass("active");
      $(this).addClass("active");
    }
  });
});
